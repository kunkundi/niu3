...
项目仅使用裸 K 价格行为策略，按完整日 K 形成结构并在盘中触发模拟买卖。在「设置与运行 → 策略与风控」配置持仓数量、仓位上限、成交费用与撮合参与率。后台根据配置采集数据、计算信号并检查模拟订单，关闭浏览器后仍会继续运行。
...
**新部署直接使用裸 K 价格行为与盘中自动交易。** 如需使用 5 分钟做 T，在「设置与运行 → 策略与风控」配置：
...
| [模拟撮合与账户规则](docs/strategy.md)           | 成交假设、仓位、费用与账本核对       |
