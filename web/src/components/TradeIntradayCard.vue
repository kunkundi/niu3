...
import { money, pct, state } from '../state'
...
const events = computed(() => props.security.events.filter((event) => event.day === day.value))
...
const chart = computed(() =>
  tradeIntradayChart(
    ...
    day.value,
    width.value,
    minuteTLevels(props.minuteT, data.value),
  ),
)
...
<template>
  <article class="trade-intraday-card" :aria-label="`${displayCode(security.name)} 成交分时图`">
    ...
    <div class="intraday-card-meta">
      <label
        ...
        ><select
          ...
          @change="chosenDay = $event.target.value"
        >
    ...
    <div v-if="chart" class="intraday-card-axes">
      ...
      <div ref="plot" class="intraday-card-plot" :style="{ height: `${chart.height}px` }">
        <svg
          ...
          role="img"
          :aria-label="`${displayCode(security.name)} ${day} 分时线，09:30至15:00，已采集至${chart.last.time}；左侧 Y 轴为价格（元），0% 水平轴对应昨收 ${money(chart.previous, 3)} 元；买卖标记按实际成交时间与价格定位`"
        >
          <path
    ...
    <div v-else class="intraday-card-empty" role="status">
      ...
      }}<button v-if="!loading" @click="load">重试</button>
    </div>
